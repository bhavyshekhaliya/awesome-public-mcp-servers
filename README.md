...
| Neither | [Official source repository](https://github.com/stonianua/neither-mcp) (`npx -y @neitherai/mcp-server`, stdio; Node 20+) | Selected project notes and documents for Cursor and Claude Desktop; retrieve related context with source evidence | Neither workspace API key (`NEITHER_API_KEY`) | `official` `knowledge` `memory` `stdio` |
...
| Provider / server | Connection details | Capabilities | Auth | Tags |
| --- | --- | --- | --- | --- |
| BulkPublish | [MCP server documentation](https://app.bulkpublish.com/docs) (`https://mcp.bulkpublish.com/mcp`, Streamable HTTP; local `npx -y @bulkpublish/mcp-server`, stdio) | Create, adapt, schedule, publish, and analyze social media content across connected channels; batch operations are supported | BulkPublish account and API key; connected social accounts and channel permissions required | `official` `social-media` `publishing` `marketing` |
...
| NotFair | [Official source and setup](https://github.com/nowork-studio/notfair-plugin) (`https://notfair.co/api/mcp/notfair`, Streamable HTTP) | Audit Google Ads, Meta Ads, X Ads, Search Console, and Analytics; make explicit approval-gated campaign changes | NotFair account and OAuth; connected provider accounts; free plan available, with paid plans and usage limits | `official` `marketing` `analytics` |
| Process Street | [MCP server guide](https://www.process.st/help/docs/mcp-server/) (`https://mcp.process.st/`, Streamable HTTP) | Workflows, workflow runs, tasks, users, data sets, reports, and operational records | Process Street account with interactive authorization, or API key as a bearer token; SAML SSO organizations are not currently supported | `official` `workflow-automation` `operations` |
...
| Worklittle Jobs | [Official source repository](https://github.com/worklittle/jobs-mcp) (`https://mcp.worklittle.com/`, Streamable HTTP) | Swipe to apply for jobs in your AI app, and search over 4 million jobs with filters like visa status, distance, and salary, and connect your Worklittle account to save jobs you love. | Worklittle account OAuth | `jobs` `search` `remote` |
...
| MuAPI | [MCP documentation](https://muapi.ai/docs/mcp) (`https://api.muapi.ai/mcp`, Streamable HTTP) | Generate and edit images, generate video, create audio, enhance media, run lipsync, and monitor prediction results through 19 tools | MuAPI API key as a Bearer token from [access keys](https://muapi.ai/access-keys); generation uses account credits | `official` `models` `multimodal` |
| Noveum | [MCP connection guide](https://noveum.ai/en/mcp) (`https://noveum.ai/api/mcp`, Streamable HTTP) | Inspect AI agent traces, create evaluation datasets, start evaluation jobs, and read results | Noveum account with OAuth 2.1 or a Bearer API key; free and paid plans, platform usage limits apply | `official` `evaluation` `tracing` |
...
| SearchLink Lite | [Official source repository](https://github.com/GlobalMatchHub/searchlink-lite) (`npx -y searchlink-lite`, stdio) | Read-only Google Search Console reporting: site overview with period comparison, performance breakdowns by query, page, country, device or date, low-CTR and position 8-20 opportunities, URL index inspection, sitemap status, on-page checks, and Google ranking update history | Google service-account or application-default credentials with Search Console read access (`GOOGLE_APPLICATION_CREDENTIALS`); no account with the maintainer and no payment | `official` `analytics` `seo` `stdio` |
...
| Provider / server | Connection details | Capabilities | Auth | Tags |
| --- | --- | --- | --- | --- |
| API.market | [Connection documentation](https://api.market/mcp) (`https://api.market/api/mcp/gateway`, Streamable HTTP) | Five tools for API discovery, operation schemas, API calls, usage and subscription management; calls and subscription changes can incur charges | API.market account with OAuth (recommended) or API key; proprietary hosted service; free tiers, paid plans and limits vary by API; [terms](https://api.market/terms_of_service), [privacy](https://api.market/privacy_policy) | `official` `apis` `gateway` |
| Brain Scanner | [Connection guide](https://brainscanner.dev/connect) (`https://brainscanner.dev/mcp/v2`, Streamable HTTP) | Project graphs, recorded context, reports and queued follow-up; versioned project metadata writes | [Account with email confirmation](https://brainscanner.dev/signup) and OAuth; free open beta; [access terms](https://brainscanner.dev/terms) and [privacy notice](https://brainscanner.dev/privacy) | `official` `project-context` `code-review` |
| ContextStream | [MCP documentation](https://contextstream.io/docs/mcp) ([official source repository](https://github.com/contextstream/mcp-server); `https://mcp.contextstream.io/mcp`, Streamable HTTP) | Shared project context for AI coding agents — code search plus decisions, lessons, and plans | ContextStream account and API key or OAuth; free tier available | `official` `project-context` `code-search` |
